...
int main() {
  ...
  while (true) {
    ...
    if (path == "/")
      send(client_socket, response1.c_str(), response1.size(), 0);
    else if (path == "/jiajun")
      send(client_socket, response2.c_str(), response2.size(), 0);
    else if (path == "/test")
      send(client_socket, response3.c_str(), response3.size(), 0);
    else
